...
impl State {
    pub fn new(
        ...
        // ELITE NIGHT LIGHT: Initialize state and spawn DBus server lock-free
        async_executor.spawn_ok(async move {
            let interface = crate::dbus::night_light::NightLightInterface;
            match zbus::connection::Builder::session() {
                Ok(builder) => match builder.name("io.github.kernel_ux.EliteNightLight").unwrap().serve_at("/io/github/kernel_ux/EliteNightLight", interface).unwrap().build().await {
                    Ok(_) => loop { std::future::pending::<()>().await; },
                    Err(e) => tracing::error!("Elite Night Light: Failed to build DBus connection: {}", e),
                },
                Err(e) => tracing::error!("Elite Night Light: Failed to connect to session bus: {}", e),
            }
        });
