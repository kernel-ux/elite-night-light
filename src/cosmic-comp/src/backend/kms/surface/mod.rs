...
fn postprocess_elements<'a>(
    ...
    if let Some(cursor_texture) = postprocess_state.cursor_texture.as_ref() {
        ...
        elements[0] = Some(TextureShaderElement::new(
            ...
            vec![
                ...
                Uniform::new(
                    "night_light_level",
                    if crate::dbus::night_light::NIGHT_LIGHT_STATE.enabled.load(std::sync::atomic::Ordering::Relaxed) {
                        crate::dbus::night_light::NIGHT_LIGHT_STATE.level.load(std::sync::atomic::Ordering::Relaxed) as f32
                    } else {
                        0.0
                    }
                ),
                Uniform::new("is_cursor", 1.0), // ELITE NIGHT LIGHT: Tell shader this is the cursor
            ],
        ));
    }
    ...
    elements[1] = Some(TextureShaderElement::new(
        ...
        vec![
            ...
            Uniform::new("is_cursor", 0.0), // ELITE NIGHT LIGHT: Tell shader this is the background
        ],
    ));
